#!/usr/bin/env python3
"""
Multi-Coin Backtest for 1-Hour Trading Model
============================================

This script runs the single trade backtest on all available coins
in the data directory to compare performance across different cryptocurrencies.

Constraints:
- Maximum 1 trade at a time
- 25 USDC position size per trade
- 26 USDC initial bankroll
- Same model trained on ETHUSDC
"""
...
class MultiCoinBacktester:
    """Backtester for multiple coins with single trade constraint."""
    ...
    def get_available_coins(self, data_dir):
        ...
        coins = []
        for file in files:
            # Extract coin name from filename (e.g., ETHUSDC_1h.csv -> ETHUSDC)
    ...
    def run_all_backtests(self, model, scaler, data_dir):
        """Run backtests on all available coins."""
    ...
    def create_summary_report(self, all_results):
        ...
        print(f"\n{'='*80}")
        print("MULTI-COIN BACKTEST SUMMARY")
        print(f"{'='*80}")
...
def main():
    """Main function to run multi-coin backtests."""
    print("Multi-Coin Backtest - 1-Hour Trading Model")
    print("=" * 60)
    print("Testing model trained on ETHUSDC across all available coins")
    ...
    backtester = MultiCoinBacktester(config)
    
    # Load model (trained on ETHUSDC)
    model, scaler = backtester.load_model(config['storage']['artifacts_dir'])
    print("Loaded model trained on ETHUSDC")
    
    # Run backtests on all coins
    all_results, all_trades = backtester.run_all_backtests(model, scaler, config['storage']['data_dir'])
    
    # Create summary report
    summary_df = backtester.create_summary_report(all_results)
    
    # Save results
    summary_df.to_csv('multi_coin_backtest_summary.csv', index=False)
    
    # Save all trades
    if all_trades:
        trades_df = pd.DataFrame(all_trades)
        trades_df.to_csv('multi_coin_backtest_all_trades.csv', index=False)
        print(f"\nAll trade details saved to: multi_coin_backtest_all_trades.csv")
    
    print(f"Summary saved to: multi_coin_backtest_summary.csv")
